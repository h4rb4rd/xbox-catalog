const formatProduct = p => {
	return {
		...
		img: `https:${
			p.LocalizedProperties[0].Images.find(
				({ ImagePurpose }) => ImagePurpose === 'Poster'
			).Uri
		}`,
		title: p.DisplaySkuAvailabilities[0].Sku.LocalizedProperties[0].SkuTitle,
		price:
			p.DisplaySkuAvailabilities[0].Availabilities[0].OrderManagementData.Price
				.ListPrice,
		msrp: p.DisplaySkuAvailabilities[0].Availabilities[0].OrderManagementData
			.Price.MSRP,
	}
}
